...
// Establecer zona horaria de Bogotá, Colombia
...
function getTurnosEmpleado($pdo) {
    try {
        ...
        if (!$id_empresa) {
            throw new Exception('No se pudo determinar la empresa del usuario', 403);
        }
        ...
        ");
        $stmt->execute([$empleado_id, $id_empresa]);
        
        if ($stmt->fetchColumn() == 0) {
            throw new Exception('Empleado no encontrado o no pertenece a su empresa', 403);
        }
        
        // Obtener turnos/horarios del empleado para la fecha específica
        ...
        $stmt = $pdo->prepare("
            SELECT 
                ...
                ehp.ID_EMPLEADO,
                ? as FECHA,
                ehp.HORA_ENTRADA as HORA_INICIO,
                ehp.HORA_SALIDA as HORA_FIN,
                ds.NOMBRE as TIPO_DIA,
                ehp.ACTIVO,
                ehp.NOMBRE_TURNO,
                TIMESTAMPDIFF(MINUTE, ehp.HORA_ENTRADA, ehp.HORA_SALIDA) / 60 as HORAS_TURNO
            FROM empleado_horario_personalizado ehp
            LEFT JOIN dia_semana ds ON ehp.ID_DIA = ds.ID_DIA
            WHERE ehp.ID_EMPLEADO = ? 
            AND ehp.ID_DIA = ?
            AND ehp.ACTIVO = 'S'
            AND ? BETWEEN ehp.FECHA_DESDE AND COALESCE(ehp.FECHA_HASTA, '2099-12-31')
            ORDER BY ehp.HORA_ENTRADA
        ");
        $stmt->execute([$fecha, $empleado_id, $dia_semana, $fecha]);
        $turnos = $stmt->fetchAll(PDO::FETCH_ASSOC);
        
        // Si no hay horarios específicos para la fecha, buscar horarios template
        if (empty($turnos)) {
            ...
            $stmt = $pdo->prepare("
                SELECT 
                    ...
                    '17:00:00' as HORA_FIN,
                    'NORMAL' as TIPO_DIA,
                    'S' as ACTIVO,
                    8.0 as HORAS_TURNO
                WHERE ? <= 5 -- Solo días laborables
            ");
            $stmt->execute([$empleado_id, $fecha, $dia_semana]);
            $turnos = $stmt->fetchAll(PDO::FETCH_ASSOC);
        }
        ...
        echo json_encode([
            ...
            'multiple_turnos' => count($turnos) > 1,
            'timestamp' => date('Y-m-d H:i:s')
        ]);
...
function getJustificacionesRecientes($pdo) {
    try {
        ...
        $pagina = intval($_GET['pagina'] ?? $_GET['page'] ?? 1);
    ...
    try {
        ...
        $sql = "
            SELECT 
                ...
                -- Información del turno/horario
                CASE 
                    WHEN j.justificar_todos_turnos = 1 THEN 'Todos los turnos'
                    WHEN j.turno_id IS NOT NULL THEN ehp.NOMBRE_TURNO
                    ELSE 'Turno no especificado'
                END as turno_nombre,
                
                CASE 
                    WHEN j.justificar_todos_turnos = 1 THEN 'Jornada completa'
                    WHEN j.turno_id IS NOT NULL THEN CONCAT(
                        DATE_FORMAT(ehp.HORA_ENTRADA, '%H:%i'), 
                        ' - ', 
                        DATE_FORMAT(ehp.HORA_SALIDA, '%H:%i')
                    )
                    ELSE 'Horario no definido'
                END as turno_horario,
                
                j.justificar_todos_turnos as es_jornada_completa
...
function createJustificacion($pdo, $data) {
    try {
        ...
        // PERMITIR SELECCIÓN FLEXIBLE DE TIPO DE FALTA (constraint eliminado)
        $tipo_falta = $data['tipo_falta'] ?? 'completa';
        
        // Validar tipos permitidos
        $tipos_validos = ['completa', 'parcial', 'tardanza'];
        if (!in_array($tipo_falta, $tipos_validos)) {
            $tipo_falta = 'completa'; // Default seguro
        }
        
        // Preparar datos para inserción
        $justificar_todos_turnos = isset($data['justificar_todos_turnos']) ? 
            (int)$data['justificar_todos_turnos'] : 0;
        
        $turno_id = isset($data['turno_id']) && !empty($data['turno_id']) ? 
            $data['turno_id'] : null;
        ...
        // Crear justificación con flexibilidad completa
        $stmt = $pdo->prepare("
            INSERT INTO justificaciones (
                empleado_id, fecha_falta, motivo, detalle_adicional,
                tipo_falta, horas_programadas, justificado_por,
                justificar_todos_turnos, turno_id, turnos_ids,
                estado, fecha_justificacion, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
        ...
        $stmt->execute([
            ...
            $turno_id,
            $turnos_ids,
            'aprobada', // Estado siempre aprobado para justificaciones desde gerencia
            $currentDateTime, // fecha_justificacion
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            SELECT
                ...
                e.DNI as empleado_codigo,
                CONCAT(e.NOMBRE, ' ', e.APELLIDO) as empleado_nombre_completo,
                -- Información de establecimiento y sede
                est.NOMBRE as establecimiento_nombre,
                sed.NOMBRE as sede_nombre,
                -- Información del turno/horario
                CASE
...
function getEmpleados($pdo) {
    try {
        $stmt = $pdo->prepare("
            ...
            LEFT JOIN establecimiento est ON e.ID_ESTABLECIMIENTO = est.ID_ESTABLECIMIENTO
            LEFT JOIN sede sed ON est.ID_SEDE = sed.ID_SEDE
            WHERE e.ACTIVO = 'S' AND e.ESTADO = 'A'
...
/**
 * Obtener justificaciones con filtros avanzados para el módulo
 */
function getJustificacionesModulo($pdo) {
    try {
        require_once __DIR__ . '/../auth/session.php';
        initSession();

        $empresaId = $_SESSION['id_empresa'] ?? ($_SESSION['empresa_id'] ?? null);
        if (!$empresaId && function_exists('getCurrentUser')) {
        ...
        if (!$empresaId) {
            echo json_encode([
                'success' => false,
                'message' => 'No se pudo determinar la empresa del usuario en sesión'
            ]);
            return;
        }
        ...
        if (!empty($_GET['empleado'])) {
            $whereConditions[] = "j.empleado_id = ?";
            $params[] = $_GET['empleado'];
        }
        
        // Búsqueda por nombre de empleado
        if (!empty($_GET['empleado_search'])) {
            $whereConditions[] = "CONCAT(e.NOMBRE, ' ', e.APELLIDO) LIKE ?";
            $params[] = '%' . $_GET['empleado_search'] . '%';
        }
        
        if (!empty($_GET['estado'])) {
            $whereConditions[] = "j.estado = ?";
            $params[] = $_GET['estado'];
        }
        
        if (!empty($_GET['fecha_desde'])) {
            $whereConditions[] = "j.fecha_falta >= ?";
        ...
        if (!empty($_GET['sede'])) {
            $whereConditions[] = "sed.ID_SEDE = ?";
            $params[] = $_GET['sede'];
        }
        
        if (!empty($_GET['establecimiento'])) {
            $whereConditions[] = "e.ID_ESTABLECIMIENTO = ?";
            $params[] = $_GET['establecimiento'];
        }
        
        // Filtro de seguridad: solo empleados de la empresa del usuario
        $whereConditions[] = "sed.ID_EMPRESA = ?";
        $params[] = $empresaId;
        ...
        $sql = "
            SELECT 
                ...
                est.NOMBRE as establecimiento_nombre,
                sed.NOMBRE as sede_nombre,
                u.NOMBRE_COMPLETO as justificado_por_nombre,
                ...
                j.justificar_todos_turnos as es_jornada_completa,
                j.turno_id as id_turno_especifico
                
            FROM justificaciones j
            LEFT JOIN empleado e ON j.empleado_id = e.ID_EMPLEADO
            LEFT JOIN establecimiento est ON e.ID_ESTABLECIMIENTO = est.ID_ESTABLECIMIENTO
            LEFT JOIN sede sed ON est.ID_SEDE = sed.ID_SEDE
            LEFT JOIN usuario u ON j.justificado_por = u.ID_USUARIO
            LEFT JOIN empleado_horario_personalizado ehp ON j.turno_id = ehp.ID_EMPLEADO_HORARIO
            $whereClause
            ORDER BY j.created_at DESC
            LIMIT 1000
        ...
        error_log("Error obteniendo justificaciones: " . $e->getMessage());
        http_response_code(500);
        echo json_encode([
            'success' => false,
            'message' => 'Error obteniendo justificaciones: ' . $e->getMessage()
...
/**
 * Buscar empleados con filtros y texto
 */
function searchEmpleados($pdo) {
    try {
        $search = $_GET['search'] ?? '';
        $sede_id = $_GET['sede_id'] ?? null;
        $establecimiento_id = $_GET['establecimiento_id'] ?? null;
        $fecha_referencia = $_GET['fecha'] ?? date('Y-m-d');
        $solo_elegibles = isset($_GET['solo_elegibles']) ? (bool)$_GET['solo_elegibles'] : true;
        ...
        $whereConditions = ["e.ACTIVO = 'S'", "e.ESTADO = 'A'"];
        $params = [];
        
        // CRÍTICO: Filtrar solo empleados de la empresa del usuario logueado
        $whereConditions[] = "sed.ID_EMPRESA = ?";
        $params[] = $id_empresa;
        
        // Búsqueda por texto
        if (!empty($search)) {
            $whereConditions[] = "(CONCAT(e.NOMBRE, ' ', e.APELLIDO) LIKE ? OR e.DNI LIKE ? OR e.CORREO LIKE ?)";
            $searchTerm = "%$search%";
            $params[] = $searchTerm;
            $params[] = $searchTerm;
            $params[] = $searchTerm;
        ...
        // Si se solicitan solo elegibles, aplicar lógica mejorada por turnos
        if ($solo_elegibles) {
            $empleados_elegibles = [];
            ...
            foreach ($empleados as $empleado) {
                ...
                // 1. Obtener turnos programados para la fecha de referencia
                $fecha_obj = new DateTime($fecha_referencia);
                $dia_semana = $fecha_obj->format('N'); // 1=Lunes, 7=Domingo
                ...
                $stmt_turnos = $pdo->prepare("
                    ...
                    ORDER BY ehp.HORA_ENTRADA
                ");
                $stmt_turnos->execute([$empleado['id'], $dia_semana, $fecha_referencia]);
                $turnos_programados = $stmt_turnos->fetchAll(PDO::FETCH_ASSOC);
                
                if (empty($turnos_programados)) {
                    // Sin horario programado para este día
                    continue;
                }
                
                // 2. Para cada turno, verificar si puede ser justificado
                foreach ($turnos_programados as $turno) {
                    $turno_disponible = true;
                    
                    // 2a. Verificar si ya tiene justificación específica para este turno
                    $stmt_just_turno = $pdo->prepare("
                        SELECT COUNT(*) FROM justificaciones 
                        WHERE empleado_id = ? 
                        AND fecha_falta = ? 
                        AND (turno_id = ? OR justificar_todos_turnos = 1)
                    ");
                    $stmt_just_turno->execute([
                        $empleado['id'], 
                        $fecha_referencia, 
                        $turno['ID_EMPLEADO_HORARIO']
                    ]);
                    $ya_justificado_turno = $stmt_just_turno->fetchColumn() > 0;
                    
                    if ($ya_justificado_turno) {
                        $turno_disponible = false;
                    }
                    
                    // 2b. Verificar si ya marcó entrada para este turno específico
                    if ($turno_disponible) {
                        $stmt_asistencia_turno = $pdo->prepare("
                            SELECT COUNT(*) FROM asistencia 
                            WHERE ID_EMPLEADO = ? 
                            AND FECHA = ?
                            AND ID_EMPLEADO_HORARIO = ?
                            AND TIPO = 'ENTRADA'
                        ");
                        $stmt_asistencia_turno->execute([
                            $empleado['id'], 
                            $fecha_referencia,
                            $turno['ID_EMPLEADO_HORARIO']
                        ]);
                        $ya_marco_entrada_turno = $stmt_asistencia_turno->fetchColumn() > 0;
                        
                        if ($ya_marco_entrada_turno) {
                            $turno_disponible = false;
                        }
                    }
                    
                    // Si el turno está disponible, agregarlo a la lista
                    if ($turno_disponible) {
                        $turnos_disponibles[] = [
                            'id_empleado_horario' => $turno['ID_EMPLEADO_HORARIO'],
                            'nombre_turno' => $turno['NOMBRE_TURNO'],
                ...
                // 3. El empleado es elegible si tiene al menos un turno disponible
                if (!empty($turnos_disponibles)) {
                    $es_elegible = true;
                    $razon_elegibilidad = count($turnos_disponibles) > 1 ? 
                        'Múltiples turnos disponibles para justificar' : 
                        'Turno disponible para justificar';
                }
                
                if ($es_elegible) {
                    // Agregar información adicional de elegibilidad
                    $empleado['es_elegible'] = true;
                    $empleado['razon_elegibilidad'] = $razon_elegibilidad;
                    $empleado['turnos_disponibles'] = $turnos_disponibles;
                    $empleado['total_turnos_disponibles'] = count($turnos_disponibles);
                    $empleados_elegibles[] = $empleado;
                }
            }
            
            $empleados = $empleados_elegibles;
        }
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Error buscando empleados: ' . $e->getMessage()
        ]);
    }
}

/**
 * Obtener establecimientos por sede
 */
function getEstablecimientos($pdo) {
    try {
        $sede_id = $_GET['sede_id'] ?? null;
        ...
        $whereConditions = ["est.ESTADO = 'A'", "sed.ID_EMPRESA = ?"];
        $params = [$id_empresa];
        
        if ($sede_id) {
            $whereConditions[] = "est.ID_SEDE = ?";
            $params[] = $sede_id;
        }
        ...
        $stmt = $pdo->prepare("
            SELECT est.ID_ESTABLECIMIENTO, est.NOMBRE, est.ID_SEDE, sed.NOMBRE as sede_nombre
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Error obteniendo establecimientos: ' . $e->getMessage()
        ]);
    }
}
?>