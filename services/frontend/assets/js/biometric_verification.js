...
function pythonServiceFetch(path, options = {}) {
    ...
    const url = path.startsWith('http') ? path : pythonServiceEndpoint(path);
    const controller = typeof AbortController !== 'undefined' ? new AbortController() : null;
    const { timeoutSeconds, forceProxy, ...fetchOptions } = options;
...
class BiometricVerificationModal {
    ...
    async checkServiceStatus() {
        try {
            const response = await pythonServiceFetch(PYTHON_SERVICE_HEALTH_URL, {
                method: 'GET',
                timeoutSeconds: 5
            });
    ...
    startFacialDetection() {
        ...
        const detectFrame = async () => {
            ...
            try {
                ...
                const response = await pythonServiceFetch('verify_face', {
                    method: 'POST',
                    headers: {
    ...
    showError(title, message, recommendations = [], technicalError = null) {
        ...
        if (technicalError) {
            document.getElementById('error-technical-content').textContent = technicalError.message || technicalError;
            document.getElementById('error-technical-details').style.display = 'block';
        } else {
            document.getElementById('error-technical-details').style.display = 'none';
        }
    ...
    // Mostrar error de validación de asistencia
    showAttendanceValidationError(reason, details = null) {
        ...
        // Determinar el tipo de error y recomendaciones específicas
        ...
        if (reason.includes('entrada abierta') || reason.includes('salida correspondiente')) {
            ...
            recommendations = [
                ...
                'Verifique su estado de asistencia actual',
                'Contacte a su supervisor si necesita ayuda'
            ];
        } else if (reason.includes('horarios disponibles') || reason.includes('completado todos los horarios')) {
            title = 'Horarios Completados';
            recommendations = [
                'Ya ha completado todos sus horarios de trabajo para hoy',
                'Si necesita trabajar horas extras, contacte a su supervisor',
                'Verifique su horario laboral asignado'
            ];
            ...
            recommendations = [
                'Han transcurrido más de 8 horas desde el inicio de su horario laboral',
                'Contacte a RRHH para gestionar horas extras',
            ...
            recommendations = [
                'No tiene entradas activas para registrar salida',
                'Verifique que haya registrado su entrada primero',
                'Contacte al administrador si hay un error'
            ];
        } else {
            recommendations = [
        ...
        // Mostrar mensaje de error usando el modal existente
        this.showError(
            title,
            reason,
            recommendations
