...
class BiometricVerificationModal {
    ...
    async checkServiceStatus() {
        try {
            const serviceUrl = window.SYNKTIME?.pythonService?.baseUrl || 'http://127.0.0.1:8000';
            const healthPath = window.SYNKTIME?.pythonService?.healthPath || 'health';
    ...
    startFacialDetection() {
        ...
        const detectFrame = async () => {
            ...
            try {
                ...
                const serviceUrl = window.SYNKTIME?.pythonService?.baseUrl || 'http://127.0.0.1:8000';
                const response = await fetch(`${serviceUrl}/verify_face`, {
                    method: 'POST',
                    headers: {
